...
def generateStochasticRSI(close_array, timeperiod):
    ...
    stochrsif, stochrsis = ta.STOCH(rsi, rsi, rsi, fastk_period=18, slowk_period=6, slowd_period=3)
    ...
    while 1:
        ...
        for i in range(len(liste)):
            try:
                ...
                stochasticRsiF_30m, stochasticRsiS_30m, close_array_30m = generateStochasticRSI_new(connection,pair,"30m",66)
                supertrend_30m = generateSuperTrend_new('30m',pair,10,3)
                print("30dklık data getirildi")

                #rsi, supertrend -> 1 saatlik
                stochasticRsiF_1h, stochasticRsiS_1h, close_array_1h = generateStochasticRSI_new(connection,pair,"1h",66)
                supertrend_1h = generateSuperTrend_new('1h',pair,10,3) 
                print("1 saatlik data getirildi")

                #rsi, supertrend -> 2 saatlik
                stochasticRsiF_2h, stochasticRsiS_2h, close_array_2h = generateStochasticRSI_new(connection,pair,"2h",66)
                supertrend_2h = generateSuperTrend_new('2h',pair,10,3)
                print("2 saatlik data getirildi")

                #rsi, supertrend -> 3 saatlik
                supertrend_3h = generateSuperTrend_new("1d",pair,10,3)
                stochasticRsiF_3h, stochasticRsiS_3h, close_array_3h = generateStochasticRSI_new(connection,pair,"1d",66)
                print("3saatlik data getirildi")

                #rsi, supertrend -> 4 saatlik
                stochasticRsiF_4h, stochasticRsiS_4h, close_array_4h = generateStochasticRSI_new(connection,pair,"4h",66)
                supertrend_4h = generateSuperTrend_new('4h',pair,10,3)
                print("4 saatlik data getirildi")
                

                if (liste_json[i]['signalSend'] == False):
                    print(pair , close_array_3h[-1])
                    print("30: ",stochasticRsiF_30m[-1], stochasticRsiS_30m[-1])
                    print("1h: ",stochasticRsiF_1h[-1], stochasticRsiS_1h[-1])
                    print("2h: ",stochasticRsiF_2h[-1],stochasticRsiS_2h[-1])
                    print("3h: ",stochasticRsiF_3h[-1], stochasticRsiS_3h[-1])
                    print("4h: ",stochasticRsiF_4h[-1], stochasticRsiS_4h[-1])
                    #15 dakikalık, 30 dakikalık, 1 saatlik, 2 saatlik, 4 saatlik grafiklerde Supertrend varsa al
                    if (close_array_3h[-1]>supertrend_1h[-1]) and (close_array_3h[-1]>supertrend_30m[-1]) and (close_array_3h[-1]>supertrend_2h[-1]) and (close_array_3h[-1]>supertrend_4h[-1]):
                        
                        print("1 saatlik, 2 saatlik, 4 saatlik grafiklerde Supertrend var - {} Fiyat: {} BUSD".format(pair,close_array_3h[-1]))

                        #15 dakikalık StochRSI değerlerinde Mavi kırmızı değerden küçükse veya eşitse ve mavi değer kırmızı değerden büyükse
                        #mavi veya kırmızı değerden biri 70 den küçük veya eşitse al
                        if (stochasticRsiF_3h[-2]<=stochasticRsiS_3h[-2]) and (stochasticRsiF_3h[-1]>stochasticRsiS_3h[-1]) and min(stochasticRsiF_3h[-1],stochasticRsiS_3h[-1])<=50:

                                print("Yeni - Super Trend Var - StochasticRSI var  - Value: ", pair ,close_array_3h[-1],datetime.now().strftime("%H:%M:%S")) 
                                msg = str("<pre> ## 3saatlik Alış Emri \n Super Trend: var \n StochasticRSI: var \n Birim: {} {}BUSD </pre>".format(pair,close_array_3h[-1]))
                                liste_json[i].update({"name":pair,"price":close_array_3h[-1],"supertrend":True,"stochrsi":True,"signalSend":True})
                                sendTelegramMsg('Telegram Bot Token',chat_id,msg)
                        else:
                            liste_json[i].update({"name":pair,"price":close_array_3h[-1],"supertrend":True,"stochrsi":False,"signalSend":False})
                    else:
                        print("SuperTrend yok")
                        liste_json[i].update({"name":pair,"price":close_array_3h[-1],"supertrend":False,"stochrsi":False,"signalSend":False})
                else:
                    print(pair)
                    print("F: ",stochasticRsiF_3h[-1])
                    print("S: ",stochasticRsiS_3h[-1])

                    if (stochasticRsiF_3h[-2]>=stochasticRsiS_3h[-2]) and (stochasticRsiF_3h[-2]>=stochasticRsiS_3h[-2]) :
